...
export default function EvaluateStaffScreen() {
  ...
  const loadList = async (isRefresh = false) => {
    try {
      ...
      const staff = await EvaluationHandler.getStaffList();
      setItems(staff);
    } catch (error) {
      console.error('Failed to load staff to evaluate:', error);
      showAlert({
        title: 'Error',
        message: 'Failed to load staff to evaluate. Please try again.',
        type: 'error',
      });
    } finally {
      if (isRefresh) {
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={[styles.loadingText, { color: colors.secondaryFont }]}>Loading staff...</Text>
          </View>
        ) : (
          <ScrollView
            ...
            <View style={styles.section}>
              {items.length === 0 ? (
                <View style={[styles.emptyContainer, { backgroundColor: colors.cardBackground }]}>
                  ...
                  <Text style={[styles.emptyTitle, { color: colors.mainFont }]}>No Staff to Evaluate</Text>
                  <Text style={[styles.emptySubtitle, { color: colors.secondaryFont }]}>
                    You don&apos;t have any pending staff evaluations right now.
                  </Text>
                </View>
              ) : (
                items.map((item) => (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      router.push({
                        ...
                        params: { type: 'staff', id: item.value, name: item.text },
                      })
                    }
                  >
